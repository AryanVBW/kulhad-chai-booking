...
export const saveCustomer = async (customer: Omit<Customer, 'id' | 'createdAt' | 'updatedAt'>): Promise<Customer | null> => {
  const { data, error } = await supabase
    .from('customers')
    .insert({
      name: customer.name,
...
export const saveProduct = async (product: Omit<Product, 'id' | 'createdAt' | 'updatedAt'>): Promise<Product | null> => {
  const { data, error } = await supabase
    .from('products')
    .insert({
      name: product.name,
...
export const saveInvoice = async (invoice: Omit<Invoice, 'id' | 'createdAt' | 'updatedAt'>): Promise<Invoice | null> => {
...
export const updateInvoice = async (id: string, updates: Partial<Invoice>): Promise<Invoice | null> => {
  const { data, error } = await supabase
    ...
    .update({
      ...
      ...(updates.subtotal && { subtotal: updates.subtotal }),
      ...(updates.discount && { discount: updates.discount }),
      ...(updates.discountType && { discount_type: updates.discountType }),
      ...(updates.taxAmount && { tax_amount: updates.taxAmount }),
      ...(updates.totalAmount && { total: updates.totalAmount }),
...
export const savePayment = async (payment: Omit<Payment, 'id' | 'createdAt'>): Promise<Payment | null> => {
  const { data, error } = await supabase
    .from('payments')
    .insert({
...
export const updateBusinessSettings = async (updates: Partial<BusinessSettings>): Promise<BusinessSettings | null> => {
  const { data, error } = await supabase
    .from('business_settings')
    .update({
...
export const generateInvoiceNumber = async (): Promise<string> => {
  ...
  await updateBusinessSettings({ invoiceCounter: newCounter })
