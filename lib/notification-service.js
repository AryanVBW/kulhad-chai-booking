...
// Lazy load services to avoid bundling server-only packages in client
let emailService, smsService, pushService;

async function getEmailService() {
  if (!emailService) {
    const module = await import('./services/email-service');
...
async function getSmsService() {
  if (!smsService) {
    const module = await import('./services/sms-service');
...
async function getPushService() {
  if (!pushService) {
    const module = await import('./services/push-service');
    pushService = module.pushService;
  }
  return pushService;
}
    ...
    try {
      ...
      switch (channel) {
        ...
        case 'sms':
          ...
          const smsSvc = await getSmsService();
          result = await smsSvc.sendSMS(
            options.phone,
            notification.message,
            notification.metadata
          );
        ...
        case 'push':
          ...
          const pushSvc = await getPushService();
          result = await pushSvc.sendPushNotification(
